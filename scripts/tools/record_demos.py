...
from isaaclab.devices import KuavoRosJointDevice, KuavoRosJointDeviceCfg    
...
def setup_kuavo_teleop_interface():
    ...
    try:
        teleop_interface = KuavoRosJointDevice(device_cfg)
...
def event_reset_delay_seconds(t: float) -> float:
    return (t - 2.0) if t >= 4.0 else 2.0
...
def handle_reset(
    ...
    if should_have_delay:
        ...
        delay = args_cli.reset_delay if delay_seconds is None else float(delay_seconds)
        delay_message = f"{label_text}\n\n🔄 Reset complete!\n⏱️  Recording starts in {delay:.1f} seconds...\n👋 Move your arms to starting position now!"
        instruction_display.show_demo(delay_message)
        
        print(f"\n✓ Reset complete!")
        print(f"⏱️  Countdown: Recording starts in {delay:.1f} seconds...")
...
def clamp_delta(curr: torch.Tensor, target: torch.Tensor, max_delta: float) -> torch.Tensor:
    # curr/target: [1, dofs] or [num_envs, dofs]
    delta = target - curr
...
def run_simulation_loop(
    ...
    def start_recording_instance():
        nonlocal reset_delay_active, reset_delay_start_time, should_start_with_delay, reset_delay_seconds
        reset_delay_seconds = args_cli.reset_delay
    ...
    kb_cfg = KuavoKeyboardDeviceCfg()
    kb_device = KuavoKeyboardDevice(kb_cfg)
        ...
        while simulation_app.is_running():
            ...
            if reset_delay_active:
                ...
                if remaining > 0:
                    ...
                    raw = actions  # [num_envs, dofs]
                    
                    # Build ready_action lazily if not set (first run)
                    if ready_action is None:
                        ready_action = env.scene["robot"].data.joint_pos[:, joint_ids].clone()
                        ready_action[:, READY_L4_IDX] = READY_ANGLE
                    ...
                    now = time.time()
                    if now - last_debug_print_t >= DEBUG_PRINT_PERIOD:
                        last_debug_print_t = now
                        print(
                            f"⏱️ {remaining:.1f}s | phase {phase} | max_err={max_err.item():.2f}"
                        )
            ...
            if running_recording_instance:
                ...
                # Subtasks: use `obs` (not `obv[0]`)
                if subtasks is not None and subtasks == {}:
                    subtasks = obs.get("subtask_terms")
            else:
                env.sim.render()
            ...
            if should_reset_recording_instance:
                success_step_count, reset_delay_active, reset_delay_start_time = handle_reset(
                    env, success_step_count, instruction_display, label_text, teleop_interface, should_start_with_delay, delay_seconds=reset_delay_seconds
                )
                ready_action = env.scene["robot"].data.joint_pos[:, joint_ids].clone()
                ready_action[:, READY_L4_IDX] = READY_ANGLE
                ready_action[:, READY_R4_IDX] = READY_ANGLE
